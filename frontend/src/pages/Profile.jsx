...
const Profile = () => {
    ...
    const fetchProfileData = async () => {
      try {
        const [quizRes, buyRes] = await Promise.all([
          ...
          axios.get(`${API_URL}/api/buy/user/${user.id}`)
        ]);
  ...
  if (loading) return <div style={{ padding: '60px', textAlign: 'center', fontFamily: "'Fredoka', sans-serif" }}>Loading Profile...</div>;
  ...
  return (
    <div style={{ background: 'var(--cream)', minHeight: '100dvh', padding: '20px', position: 'relative' }}>
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', fontFamily: "'Fredoka', sans-serif" }}>
        ...
        <div style={{ background: 'white', padding: '40px', borderRadius: '20px', boxShadow: 'var(--shadow)', marginBottom: '30px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '30px' }}>
          <div style={{ width: '100px', height: '100px', borderRadius: '50%', background: 'linear-gradient(135deg, var(--orange), var(--orange-light))', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '40px', fontWeight: 800 }}>
            {user.name.charAt(0).toUpperCase()}
          </div>
          <div style={{ flex: '1 1 200px' }}>
            <h2 style={{ color: 'var(--brown)', fontSize: '32px', margin: '0 0 10px 0' }}>{user.name}</h2>
          ...
          <button onClick={handleLogout} style={{ padding: '10px 20px', background: '#ffebee', color: '#c62828', border: 'none', borderRadius: '50px', fontWeight: 700, cursor: 'pointer', width: 'max-content' }}>Logout</button>
        </div>
        ...
        {petProfile ? (
          <div style={{ background: 'white', padding: '30px', borderRadius: '20px', boxShadow: 'var(--shadow)', marginBottom: '30px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid rgba(0,0,0,0.05)', paddingBottom: '15px', marginBottom: '20px' }}>
              <h3 style={{ color: 'var(--brown)', margin: 0, fontSize: '24px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                🐾 {petProfile.name}'s Profile
              </h3>
              <span style={{ background: 'var(--orange-pale)', color: 'var(--orange)', padding: '5px 15px', borderRadius: '50px', fontSize: '13px', fontWeight: 'bold' }}>
                {petProfile.gender}
              </span>
            </div>
            ...
            {petProfile.vaccines && petProfile.vaccines.length > 0 && (
              <div style={{ marginTop: '25px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '30px' }}>
                <div>
                  <h4 style={{ color: 'var(--brown)', fontSize: '16px', marginBottom: '15px', borderBottom: '1px solid #EAE4DE', paddingBottom: '5px' }}>
                    💉 Vaccination History
                  </h4>
          ...
          <div style={{ background: 'white', padding: '30px', borderRadius: '20px', boxShadow: 'var(--shadow)', marginBottom: '30px', textAlign: 'center' }}>
            <h3 style={{ color: 'var(--brown)', marginBottom: '10px', fontSize: '20px' }}>🐾 Already a dog owner?</h3>
            ...
            <button 
              ...
              style={{
                ...
                cursor: 'pointer', fontFamily: "'Fredoka', sans-serif", fontSize: '14px',
        ...
        <div style={{ background: 'white', padding: '20px', borderRadius: '20px', boxShadow: 'var(--shadow)', marginBottom: '30px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '20px' }}>
          ...
          <div style={{ color: 'var(--text-soft)', fontSize: '14px', lineHeight: 1.6, fontFamily: "'Fredoka', sans-serif", flex: '1 1 250px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '30px' }}>
          ...
          <div style={{ background: 'white', padding: '30px', borderRadius: '20px', boxShadow: 'var(--shadow)' }}>
            <h3 style={{ color: 'var(--brown)', marginBottom: '20px', borderBottom: '2px solid rgba(0,0,0,0.05)', paddingBottom: '10px' }}>Past Quizzes</h3>
          ...
          <div style={{ background: 'white', padding: '30px', borderRadius: '20px', boxShadow: 'var(--shadow)' }}>
            <h3 style={{ color: 'var(--brown)', marginBottom: '20px', borderBottom: '2px solid rgba(0,0,0,0.05)', paddingBottom: '10px' }}>Buy Interests</h3>
            {buyRequests.length > 0 ? buyRequests.map((req, i) => (
              <div key={i} style={{ marginBottom: '15px', padding: '15px', background: 'var(--cream)', borderRadius: '15px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '5px' }}>
                  ...
                  <span style={{ background: req.status === 'NEW' ? '#fff3e0' : '#e8f5e9', color: req.status === 'NEW' ? '#ef6c00' : '#2e7d32', padding: '2px 8px', borderRadius: '50px', fontSize: '11px', fontWeight: 700 }}>{req.status}</span>
                </div>
