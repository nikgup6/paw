...
const BreedProfileModal = ({ breed, user, onClose, answers, topBreeds = [], initialView = 'profile' }) => {
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, fontFamily: 'var(--font-body-family)', padding: '20px' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: 'white', padding: '30px', borderRadius: '20px', width: '100%', maxWidth: '800px', maxHeight: '90dvh', overflowY: 'auto', position: 'relative' }}>
        ...
        {showAuthWarning && (
          <div style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(255,255,255,0.95)', zIndex: 50, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderRadius: '20px', padding: '20px', textAlign: 'center' }}>
            <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 'var(--weight-semibold)', color: '#c62828', marginBottom: '15px', fontSize: '24px' }}>Login Required</h3>
            ...
            <div style={{ display: 'flex', gap: '15px' }}>
              <button onClick={() => navigate('/login')} style={{ padding: '12px 25px', background: 'var(--orange)', color: 'white', border: 'none', borderRadius: '50px', fontFamily: 'var(--font-display)', fontWeight: 'var(--weight-semibold)', cursor: 'pointer' }}>Go to Login</button>
              <button onClick={() => setShowAuthWarning(false)} style={{ padding: '12px 25px', background: 'transparent', color: 'var(--text-soft)', border: '1px solid #ddd', borderRadius: '50px', fontFamily: 'var(--font-display)', fontWeight: 'var(--weight-semibold)', cursor: 'pointer' }}>Cancel</button>
            </div>
          </div>
        )}
        ...
        {view === 'success' && (
          <div style={{ textAlign: 'center', padding: '40px' }}>
            ...
            <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 'var(--weight-semibold)', color: 'var(--orange)', marginBottom: '15px' }}>We've opened WhatsApp for you!</h3>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap', marginTop: '20px' }}>
              ...
              <button onClick={onClose} style={{ padding: '12px 30px', background: 'var(--cream)', color: 'var(--brown)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: '50px', cursor: 'pointer', fontFamily: 'var(--font-display)', fontWeight: 'var(--weight-semibold)' }}>Close</button>
            </div>
          </div>
        )}
        ...
        {view === 'profile' && (
          <div style={{ display: 'flex', gap: '30px', flexWrap: 'wrap' }}>
            <div style={{ flex: '1 1 250px' }}>
              ...
              {/* High-emphasis CTA — "Buy Now" is a literal role-table example. */}
              <button onClick={handleBuyClick} style={{ width: '100%', marginTop: '20px', padding: '15px', background: 'var(--orange)', color: 'white', border: 'none', borderRadius: '12px', fontFamily: 'var(--font-display)', fontWeight: 'var(--weight-bold)', cursor: 'pointer', fontSize: '18px', boxShadow: '0 4px 15px rgba(255,107,43,0.3)' }}>Buy Now</button>

              <div style={{ marginTop: '20px', padding: '15px', background: 'var(--cream)', borderRadius: '15px' }}>
                <h4 style={{ fontFamily: 'var(--font-display)', fontWeight: 'var(--weight-semibold)', margin: '0 0 10px 0', color: 'var(--orange)' }}>Quick Stats</h4>
              ...
              <div style={{ marginTop: '15px', padding: '15px', background: '#ffebee', borderRadius: '15px' }}>
                 <h4 style={{ fontFamily: 'var(--font-display)', fontWeight: 'var(--weight-semibold)', margin: '0 0 10px 0', color: '#c62828' }}>Monthly Cost Estimate</h4>
                 <div style={{ margin: 0, color: '#c62828', fontSize: '14px', display: 'flex', flexDirection: 'column', gap: '5px' }}>
                   {(() => {
                     ...
                     if (breakdownItems.length === 0) {
                       return <div style={{ fontWeight: 'var(--weight-bold)', fontSize: '16px' }}>{costStr}</div>;
                     }

                     return (
                       <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                         <div style={{ fontSize: '18px', fontWeight: 'var(--weight-bold)', marginBottom: '5px', textAlign: 'center', background: 'rgba(198, 40, 40, 0.1)', padding: '10px', borderRadius: '10px' }}>
                           {mainCost} <span style={{ fontSize: '12px', fontWeight: 'var(--weight-regular)' }}>/ month</span>
                         </div>
            ...
            <div style={{ flex: '2 1 300px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
                <div style={{ background: 'var(--cream)', padding: '15px', borderRadius: '15px' }}>
                  <h4 style={{ fontFamily: 'var(--font-display)', fontWeight: 'var(--weight-semibold)', color: 'var(--orange)', marginBottom: '8px' }}>Ideal Indian Cities</h4>
                  <p style={{ fontSize: '14px', margin: 0, color: 'var(--text-soft)' }}>{breed.idealCities}</p>
                </div>
                <div style={{ background: 'var(--cream)', padding: '15px', borderRadius: '15px' }}>
                  <h4 style={{ fontFamily: 'var(--font-display)', fontWeight: 'var(--weight-semibold)', color: 'var(--orange)', marginBottom: '8px' }}>Common Health Issues</h4>
